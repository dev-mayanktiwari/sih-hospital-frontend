    ...
    useEffect(() => {
        const fetchDepartments = async () => {
            try {
                ...
                const response = await axios.get('http://localhost:3000/api/v1/hospital/departments', {
                    headers: {
    ...
    return (
        <div className="container mx-auto px-4 py-8 bg-gray-100 min-h-screen">
            ...
            <div className="max-w-lg mx-auto bg-white rounded-lg shadow-md p-6">
                <h2 className="text-xl font-bold mb-4 text-blue-600">List of Departments</h2>
                ...
                <ul className="space-y-4">
                    {departments && departments.length > 0 ? (
                        departments.map((department) => (
                            ...
                            >
                                <h3 className="text-lg font-bold text-blue-600">{department.name}</h3>
                                <p className="text-gray-700">
                                    Hospital ID: {department.hospitalId}
                                </p>
                                <p className="text-gray-700">
                                    Head of Department: {department.head ? department.head : 'Not Assigned'}
                                </p>
                            </li>
                        ))
                    ) : (
